...
import styles from './EngagementPane.module.css'; // Add this import
...
const EngagementPane: React.FC<EngagementPaneProps> = ({
  ...
  return (
    <div className={`${styles.engagementPane} ${typeClass} ${className}`}>
      {/* Community Voting (only for community content) - MOVED TO TOP */}
      {contentType === 'community' && votingInfo && (
        <CommunityVoting
          upvotes={votingInfo.upvotes || 0}
          downvotes={votingInfo.downvotes || 0}
          percentage={votingInfo.percentage || 0}
          onVoteUp={onVoteUp}
      ...
      {engagementMetrics && (
        <ChainReactionPanel
          ...
          onReaction={(type, isPowerUp) => 
            console.log(`Reaction: ${type}, PowerUp: ${isPowerUp}`)
          }
        />
      )}
