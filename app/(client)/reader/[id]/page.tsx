...
export default async function ArticlePage({ params }: { params: Promise<{ id: string }> }) {
 ...
 useEffect(() => {
   const loadArticle = async () => {
     ...
     try {
       // Load the article
       const articleData = null; // Article not available in production
       if (!articleData) {
       ...
       // Skip record view in production
       
       // Get user engagement with this article
       const engagement = { hasVoted: false, hasTipped: false }; // Default engagement
 ...
 const handleVote = async (voteType: 'up' | 'down' | 'emoji', emoji?: string) => {
   ...
   try {
     ...
     // Skip engagement refresh in production
   } catch (err) {
